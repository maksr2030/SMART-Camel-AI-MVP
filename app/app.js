(function(){
  ...
  const registryCheck=core.validateCamelRegistry(data.camels);
  if(!registryCheck.valid) throw new Error('Synthetic camel registry failed validation: '+registryCheck.errors.join('; '));

  let lang='ar';
  let audit=[];
  let auctionState=core.createAuctionState('AUC-DEMO-001',1250000);
  const $=(id)=>document.getElementById(id);
  const t=(key)=>translations[lang][key]||key;

  function logEvent(ar,en,type,reference){
    audit=core.appendAuditEvent(audit,{type,reference:reference||'',messageAr:ar,messageEn:en,time:new Date()});
    renderAudit();
  }
  ...
  function renderAlerts(){
    ...
    const position={lat:24.701,lng:46.675};
    const geo=core.evaluateGeofence(position,boundary,0.002);
    const geoAlert={
      level:geo.state==='outside'?'high':(geo.state==='near'?'low':'medium'),
      titleAr:geo.state==='near'?'اقتراب من سياج جغرافي':(geo.state==='outside'?'خروج من السياج الجغرافي':'داخل النطاق الجغرافي'),
      titleEn:geo.state==='near'?'Geofence proximity':(geo.state==='outside'?'Outside geofence':'Inside geofence'),
      textAr:`حالة جغرافية تجريبية: ${geo.state}، المسافة الحدية ${geo.distanceToBoundary}.`,
      textEn:`Synthetic geofence state: ${geo.state}; boundary distance ${geo.distanceToBoundary}.`
    };
    const baseAlerts=data.alerts.filter(a=>!/geofence/i.test(a.titleEn));
    $('alertsList').innerHTML=[...baseAlerts,geoAlert].map(a=>`<div class="alert ${a.level}"><strong>${lang==='ar'?a.titleAr:a.titleEn}</strong><small>${lang==='ar'?a.textAr:a.textEn}</small></div>`).join('');
  }
  ...
  function renderMazayen(){
    const score=core.calculateMazayenScore(92,90,94);
    const el=document.querySelector('.scoreline strong');
  ...
  function renderAudit(){
    ...
    $('auditLog').innerHTML=audit.slice(0,8).map(e=>`<div class="audit-item"><time>${e.time?new Date(e.time).toLocaleTimeString(lang==='ar'?'ar-SA':'en-GB'):'n/a'}</time><span>${lang==='ar'?e.messageAr:e.messageEn}</span><small>#${e.sequence} ${e.type}${e.reference?' · '+e.reference:''}</small></div>`).join('');
  }

  function renderAll(){renderTranslations();renderKpis();renderCamels();renderAlerts();renderFeatures();calculateRisk();renderMazayen();renderAudit();}

  $('langToggle').addEventListener('click',()=>{lang=lang==='ar'?'en':'ar';renderAll();logEvent('تم تغيير اللغة إلى '+(lang==='ar'?'العربية':'الإنجليزية'),'Language changed to '+(lang==='ar'?'Arabic':'English'),'ui','LANG');});
  ...
  $('bidButton').addEventListener('click',()=>{
    const nextBid=auctionState.currentBid+25000;
    auctionState=core.placeDemoBid(auctionState,nextBid);
    $('bidResult').textContent=lang==='ar'?`تم تسجيل مزايدة تجريبية بقيمة ${auctionState.currentBid.toLocaleString('ar-SA')} ريال سعودي.`:`A demonstration bid of SAR ${auctionState.currentBid.toLocaleString('en-US')} was recorded.`;
    logEvent('تم تنفيذ مزايدة تجريبية وتسجيلها في سجل الأحداث.','A demonstration bid was executed and recorded in the event trail.','auction',auctionState.id);
  });
  $('verifyButton').addEventListener('click',()=>{const ok=core.verifyDemoCertificate($('verifyCode').value);$('verifyResult').textContent=t(ok?'verified':'notVerified');logEvent(ok?'تم التحقق من الشهادة التجريبية لـ CAMEL-001.':'فشل التحقق من رمز غير موجود في بيانات العرض.',ok?'The synthetic certificate for CAMEL-001 was verified.':'Verification failed for a code not present in the demo dataset.','verification','CAMEL-001');});
