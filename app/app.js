(function(){
  ...
  function logEvent(ar,en,type){
    audit.unshift({time:new Date(),ar,en,type});
    renderAudit();
  }
  ...
  function renderKpis(){
    $('kpis').innerHTML=data.metrics.map(m=>`<article class="kpi"><strong>${m.value}</strong><span>${t(m.key)}</span></article>`).join('');
  }
  ...
  function renderAlerts(){
    $('alertsList').innerHTML=data.alerts.map(a=>`<div class="alert ${a.level}"><strong>${lang==='ar'?a.titleAr:a.titleEn}</strong><small>${lang==='ar'?a.textAr:a.textEn}</small></div>`).join('');
  }
  ...
  function renderFeatures(){
    ...
    const filtered=data.features.filter(f=>{
      const text=(f.id+' '+f.nameAr+' '+f.nameEn+' '+f.domain).toLowerCase();
      return (status==='all'||f.status===status)&&(!q||text.includes(q));
    });
    ...
    const counts={};data.features.forEach(f=>counts[f.status]=(counts[f.status]||0)+1);
  ...
  function calculateRisk(){
    ...
    score+=Math.max(0,60-activity)*0.65;
    score+=Math.max(0,70-hydration)*0.8;
    score=Math.min(100,Math.round(score));
    let label='low',explain='riskLow';if(score>=35){label='medium';explain='riskMedium';}if(score>=65){label='high';explain='riskHigh';}
    $('riskScore').textContent=score;$('riskLabel').textContent=t(label);$('riskExplanation').textContent=t(explain);
  }
  ...
  $('verifyButton').addEventListener('click',()=>{const ok=$('verifyCode').value.trim().toUpperCase()==='CAMEL-001';$('verifyResult').textContent=t(ok?'verified':'notVerified');logEvent(ok?'تم التحقق من الشهادة التجريبية لـ CAMEL-001.':'فشل التحقق من رمز غير موجود في بيانات العرض.',ok?'The synthetic certificate for CAMEL-001 was verified.':'Verification failed for a code not present in the demo dataset.','verification');});
